"""Production entrypoint for deterministic USD/JPY and Gold technical strategies."""
...
def build_report(name: str, symbol: str, daily: pd.DataFrame, h1: pd.DataFrame) -> str:
    ...
    ds, hs4, hs1 = timeframe_snapshot(daily), timeframe_snapshot(h4), timeframe_snapshot(h1)
    p = plan.to_dict()

    direction = {"buy": "買い", "sell": "売り", "wait": "見送り"}[p["direction"]]
    lines = [
        f"【{name}】テクニカル戦略 ({RUN_TS} JST)",
        f"現在値: {fmt_price(hs1['close'], symbol)}",
        "",
        "■環境認識",
        f"日足: {ds['trend']} / 4H: {hs4['trend']} / 1H: {hs1['trend']}",
    ...
    if p["entry_price"] is not None:
        lines += [
            f"エントリー: {fmt_price(p['entry_price'], symbol)} ({p['entry_type']})",
        ...
        lines.append("エントリー条件: 現時点では優位性不足。無理に入らない")
    lines += [
        "",
        "■根拠",
        *[f"・{reason}" for reason in p["reasons"]],
        ...
        "※これはテクニカル分析による戦略候補であり、利益を保証する予測ではありません。",
    ]
    return "\n".join(lines)
