...
def _dow_snapshots(df: pd.DataFrame, cfg: StrategyConfig) -> list[dict]:
    ...
    for i in range(len(df)):
        c = i - right
        if c >= left:
            hw = hi[c - left : c + right + 1]
            lw = lo[c - left : c + right + 1]
            if hi[c] == np.max(hw) and hi[c] > np.max(np.delete(hw, left)):
                highs.append((idx[c + right], float(hi[c])))
            if lo[c] == np.min(lw) and lo[c] < np.min(np.delete(lw, left)):
                lows.append((idx[c + right], float(lo[c])))
        hv = [v for _, v in highs]
        lv = [v for _, v in lows]
        trend = "transition"
        if len(hv) >= 2 and len(lv) >= 2:
            if hv[-1] > hv[-2] and lv[-1] > lv[-2]:
                trend = "up"
            elif hv[-1] < hv[-2] and lv[-1] < lv[-2]:
                trend = "down"
            else:
                trend = "range"
        elif len(hv) >= 2 and hv[-1] > hv[-2]:
            trend = "transition"
        elif len(lv) >= 2 and lv[-1] < lv[-2]:
            trend = "transition"
        close = float(df["Close"].iloc[i])
        lh = hv[-1] if hv else None
        ll = lv[-1] if lv else None
...
def _snapshot(ind: pd.DataFrame, dow: dict, pos: int) -> dict:
    r = ind.iloc[pos]
    return {
        "trend": dow["trend"], "dow_trend": dow["dow_trend"], "dow_breakout": dow["dow_breakout"],
        "last_swing_high": dow["last_swing_high"], "last_swing_low": dow["last_swing_low"],
        "close": float(r["Close"]), "ema20": float(r["ema20"]), "ema50": float(r["ema50"]),
        "ema200": float(r["ema200"]), "rsi": float(r["rsi"]),
...
def _merge(daily: dict, h4: dict, h1: dict) -> tuple[int, list[str], str]:
    score = 0
    reasons: list[str] = []
    regs = [daily["dow_trend"], h4["dow_trend"], h1["dow_trend"]]
    regime = "range"
    if regs.count("up") >= 2:
        score += 4; reasons.append("ダウ構造が複数時間足で高値・安値切り上げ"); regime = "up"
    elif regs.count("down") >= 2:
        score += 4; reasons.append("ダウ構造が複数時間足で高値・安値切り下げ"); regime = "down"
    elif regs.count("transition") >= 2:
    ...
    if daily["dow_trend"] == h4["dow_trend"] == h1["dow_trend"] and daily["dow_trend"] in {"up", "down"}:
        score += 3; reasons.append("日足・4H・1Hのダウ構造が完全一致")
    if regime == "up" and h4["last_swing_high"] is not None and h4["close"] > h4["last_swing_high"]:
        score += 1; reasons.append("4Hで直近スイング高値を上抜け")
    if regime == "down" and h4["last_swing_low"] is not None and h4["close"] < h4["last_swing_low"]:
        score += 1; reasons.append("4Hで直近スイング安値を下抜け")
    if h4["adx"] >= 25:
        score += 2; reasons.append("4H ADXが25以上")
    elif h4["adx"] >= 18:
        score += 1; reasons.append("4H ADXが18以上")
    return score, reasons, regime


def _prepared_plan(daily, h4, h1, row, cfg):
    score, reasons, regime = _merge(daily, h4, h1)
    ...
    if regime == "up":
        ...
        return TradePlan(direction="wait", score=score, confidence="low", regime=regime, entry_type="wait",
                         entry_price=None, stop_price=None, target1=None, target2=None, risk_per_unit=None,
                         reward_r1=None, reward_r2=None,
                         reasons=tuple(reasons + ["ダウ理論のトレンド構造が未確定。転換を先回りしない"]),
                         invalidation="新しい高値・安値の構造が確認されるまで見送り")
    if score < cfg.min_score:
        return TradePlan(direction="wait", score=score, confidence="low", regime=regime, entry_type="conditional",
                         entry_price=None, stop_price=None, target1=None, target2=None, risk_per_unit=None,
                         reward_r1=None, reward_r2=None,
                         reasons=tuple(reasons + [f"スコア{score}で最低基準{cfg.min_score}未満"]),
                         invalidation="ダウ構造を維持しつつ確認条件を満たすまで見送り")
    ...
    if direction == "buy":
        ...
        stop = min(stops); risk = entry - stop
        t1 = entry + risk * cfg.target1_r; t2 = entry + risk * cfg.target2_r
        inv = (f"1H終値が直近ダウ安値 {h1['last_swing_low']:.5f} を明確に割る、または4Hダウ構造が下降へ転換"
               if h1["last_swing_low"] is not None else "1Hダウ構造が下降へ転換、または直近安値を明確に下抜け")
    else:
        entry = close - buf
        stops = [close + atr * cfg.stop_atr]
        if float(row["ema50"]) > entry: stops.append(float(row["ema50"]))
        ...
        inv = (f"1H終値が直近ダウ高値 {h1['last_swing_high']:.5f} を明確に上回る、または4Hダウ構造が上昇へ転換"
               if h1["last_swing_high"] is not None else "1Hダウ構造が上昇へ転換、または直近高値を明確に上抜け")
    if risk <= 0 or not np.isfinite(risk):
        return TradePlan(direction="wait", score=score, confidence="low", regime=regime, entry_type="wait",
                         entry_price=None, stop_price=None, target1=None, target2=None, risk_per_unit=None,
                         reward_r1=None, reward_r2=None,
                         reasons=tuple(reasons + ["リスク幅を正常に計算できない"]),
                         invalidation="有効なダウ構造と損切り位置が確定するまで見送り")
    return TradePlan(direction=direction, score=score, confidence="high" if score >= cfg.strong_score else "medium",
                     regime=regime, entry_type="immediate" if score >= cfg.strong_score else "conditional",
                     entry_price=round(entry, 5), stop_price=round(stop, 5), target1=round(t1, 5), target2=round(t2, 5),
                     risk_per_unit=round(risk, 5), reward_r1=cfg.target1_r, reward_r2=cfg.target2_r,
...
def evaluate(symbol, h1, cfg=StrategyConfig(), max_hold_bars=48, entry_valid_bars=3):
    ...
    h1d = _dow_snapshots(h1i, cfg); h4d = _dow_snapshots(h4i, cfg); dd = _dow_snapshots(dailyi, cfg)
    ...
    trades = len(outcomes); gp = sum(wins); gl = abs(sum(losses)); streak = maxstreak = 0
    for x in outcomes:
